 ...
 */
export class FirstSentenceOperation extends ChoiceOperation {
  static override isAvailable(operationSite: OperationSite) {
    return operationSite === OperationSite.START_OF_SECTION;
  }
  ...
  static override id = OperationType.FIRST_SENTENCE;
  ...
  async run() {
    ...
    const params = this.dataProcessor.firstSentence(operationData);
    const choices = await this.getModel().firstSentence(params);
